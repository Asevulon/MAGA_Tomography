...
void SourceDialog::GetPicturesPresets()
{
	...
	CFileFind finder2;
	auto temp2 = PresetsPath + _T("pic\\*.bmp");
	bWorking = finder2.FindFile(temp2);
	while (bWorking)
	{
		bWorking = finder2.FindNextFileW();
		PicsList.AddString(finder2.GetFileName());
	}
}
